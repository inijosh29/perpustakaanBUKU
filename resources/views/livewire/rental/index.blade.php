<div style="max-width:1100px;margin:auto;padding:25px;color:black;">
    ...
    @if(session()->has('success'))
        <div style="background:#16a34a;color:white;padding:14px 20px;border-radius:12px;margin-bottom:20px;box-shadow:0 10px 20px rgba(0,0,0,.15);">
            {{ session('success') }}
        </div>
    @endif

    @if(session()->has('error'))
        <div style="background:#dc2626;color:white;padding:14px 20px;border-radius:12px;margin-bottom:20px;box-shadow:0 10px 20px rgba(0,0,0,.15);">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(420px,1fr));gap:20px;">
        ...
        <div
            style="background:white;border-radius:18px;padding:20px;box-shadow:0 15px 30px rgba(0,0,0,.08);transition:.25s;"
        ...
        >
            <div style="display:flex;justify-content:space-between;gap:15px;">
                ...
                <div style="text-align:right;font-size:13px;color:#6b7280;">
                    <div>📅 Pinjam:<br><b>{{ \Carbon\Carbon::parse($rental->rented_at)->format('d M Y H:i') }}</b></div>
                    <div style="margin-top:6px;">📦 Kembali:<br>
                        <b>
                            {{ $rental->returned_at ? \Carbon\Carbon::parse($rental->returned_at)->format('d M Y H:i') : '-' }}
                        </b>
                    </div>
            ...
            <div style="margin-top:18px;text-align:right;">
                <button
                    ...
                    style="background:#2563eb;color:white;font-weight:700;padding:10px 22px;border-radius:14px;border:none;cursor:pointer;transition:.2s;"
